...
### F-11: アウトラインのアクティブ項目ハイライト
- 現在表示中のセクションに対応するアウトラインリンクに下線を付与してハイライトする
- パンくずナビゲーションと連動し、同じスクロール位置判定ロジックを共有する（`showBreadcrumb` 設定に関わらず常に動作）
- 複数レベルの見出しが同時にアクティブになる場合は、すべてのレベルのリンクをハイライトする
- アクティブな見出しが表示範囲外の場合、サイドバーを自動スクロールしてリンクを表示する（アクティブリンクがサイドバー上端から 25% の位置になるよう調整）
